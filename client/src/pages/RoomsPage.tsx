  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight" style={{ fontFamily: "'Georgia', serif" }}>Xonalar</h2>
          <p className="text-white/30 text-sm mt-1">Barcha xonalarni boshqaring</p>
        </div>
      ...
      {filtered.length === 0 && (filterFloor || filterStatus) && (
        <div className="bg-[#161923] rounded-2xl border border-white/[0.06] p-12 text-center">
          ...
          <p className="text-white/30 text-sm">Filtrga mos xona topilmadi</p>
          <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {floorRooms.map(room => {
                ...
                return (
                  <div
                    key={room.id}
                    className={`${style.bg} border ${style.border} rounded-2xl p-4 hover:scale-[1.01] transition-all cursor-pointer group`}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className={`text-[11px] ${style.text} font-medium`}>{STATUS_LABELS[room.status]}</p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-black/20 rounded-xl p-3 space-y-1.5">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center">
                            <span className="text-[10px] font-bold text-blue-400">{room.guestName[0]}</span>
                          </div>
                          <p className="text-sm font-medium">{room.guestName}</p>
                        </div>
                        {room.checkIn && (
                          <p className="text-[10px] text-white/25 pl-8">
                    ...
                    {room.status === 'available' && (
                      <div className="flex items-center gap-2 text-emerald-400/60 group-hover:text-emerald-400 transition-colors">
                        ...
                        <span className="text-xs">Joylashtirish uchun bosing</span>
                      </div>
                    )}
                    {room.status === 'occupied' && (
                      <div className="flex items-center gap-2 text-blue-400/60 group-hover:text-blue-400 transition-colors mt-2">
                        <CheckOutIcon className="w-3.5 h-3.5" />
                        <span className="text-xs">Chiqarish uchun bosing</span>
                      </div>
                    )}
                    {room.status === 'booked' && (
                      <div className="bg-purple-500/5 rounded-lg p-2 mt-1">
                        <div className="flex items-center gap-2 text-purple-400/70">
                          <CalendarCheck className="w-3.5 h-3.5" />
                          <span className="text-xs">{room.guestName} — bron qilingan</span>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3 space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-xs text-white/40">Jami to'lov:</span>
                <span className="text-lg font-bold text-emerald-400">
                  {formatUZS(Number(priceOverride) || 0)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-white/40">Muddat:</span>
      ...
      <Dialog open={dialogMode === 'checkout'} onOpenChange={() => closeDialog()}>
        <DialogContent className="bg-[#1a1d2a] border-white/[0.08] text-white max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <CheckOutIcon className="w-5 h-5 text-blue-400" />
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="flex flex-col sm:flex-row sm:grid sm:grid-cols-3 gap-2">
                ...
                <button
                  ...
                  disabled={submitting}
                  className="py-3 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 text-sm font-medium hover:bg-purple-500/20 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  disabled={submitting}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold py-3 rounded-xl shadow-lg shadow-blue-500/20 text-sm disabled:opacity-50"
                >
                  {submitting ? 'Chiqarilmoqda...' : 'Chiqarish'}
                </button>
              </div>
            </div>
          )}
      ...
      <Dialog open={dialogMode === 'status'} onOpenChange={() => closeDialog()}>
        <DialogContent className="bg-[#1a1d2a] border-white/[0.08] text-white max-w-sm">
          <DialogHeader>
            <DialogTitle>{selectedRoom?.number}-xona holatini o'zgartirish</DialogTitle>
          </DialogHeader>
...
function Field({ label, value, onChange, placeholder, type = 'text', min }: {
  ...
  return (
    <div>
      <label className="text-[11px] text-white/40 uppercase tracking-wider font-medium block mb-1.5">{label}</label>
      <input
        type={type}
        value={value}
        ...
        min={min}
        className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-amber-500/40 transition-all"
      />
    </div>
  )
}
