    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight" style={{ fontFamily: "'Georgia', serif" }}>Moliya</h2>
          <p className="text-white/30 text-xs sm:text-sm mt-0.5 sm:mt-1">Kirim va chiqimlarni boshqaring</p>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          disabled={todayIsClosed}
          className={`flex items-center gap-1.5 sm:gap-2 font-semibold px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl shadow-lg transition-all text-xs sm:text-sm shrink-0 ${
            todayIsClosed
        ...
        >
          {todayIsClosed ? <Lock className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
          {todayIsClosed ? 'Yopilgan' : "Qo'shish"}
        </button>
      </div>

      {todayIsClosed && (
        <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5">
          <Lock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-400 shrink-0" />
          <span className="text-[11px] sm:text-sm text-amber-400/80">
      ...
      <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
        <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/10 rounded-xl sm:rounded-2xl p-2 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-2">
            ...
            <span className="text-[8px] sm:text-[10px] text-emerald-400/60 uppercase tracking-wider">Kirim</span>
          </div>
          <p className="text-xs sm:text-2xl font-bold text-emerald-400 truncate">{formatUZS(totalIncome)}</p>
        </div>
        <div className="bg-gradient-to-br from-red-500/10 to-red-500/5 border border-red-500/10 rounded-xl sm:rounded-2xl p-2 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-2">
            <TrendingDown className="w-3 h-3 sm:w-4 sm:h-4 text-red-400 shrink-0" />
            <span className="text-[8px] sm:text-[10px] text-red-400/60 uppercase tracking-wider">Chiqim</span>
          </div>
          <p className="text-xs sm:text-2xl font-bold text-red-400 truncate">{formatUZS(totalExpense)}</p>
        </div>
        <div className="bg-gradient-to-br from-amber-500/10 to-amber-500/5 border border-amber-500/10 rounded-xl sm:rounded-2xl p-2 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-2">
            <Wallet className="w-3 h-3 sm:w-4 sm:h-4 text-amber-400 shrink-0" />
            <span className="text-[8px] sm:text-[10px] text-amber-400/60 uppercase tracking-wider">Balans</span>
          </div>
          <p className={`text-xs sm:text-2xl font-bold truncate ${totalIncome - totalExpense >= 0 ? 'text-amber-400' : 'text-red-400'}`}>
      ...
      <div className="space-y-2">
        <div className="relative w-full sm:max-w-xs">
          <Search className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/20 absolute left-3 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder="Qidirish..."
            className="w-full bg-[#161923] border border-white/[0.06] rounded-xl pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-2.5 text-xs sm:text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-amber-500/30"
          />
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          <div className="relative">
            <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white/20 absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2" />
            <input
              type="date"
              value={dateFilter}
              onChange={e => setDateFilter(e.target.value)}
              className="bg-[#161923] border border-white/[0.06] rounded-xl pl-7 sm:pl-9 pr-2 sm:pr-3 py-1.5 sm:py-2 text-[10px] sm:text-xs text-white/50 focus:outline-none focus:border-amber-500/30"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#161923] rounded-2xl border border-white/[0.06] overflow-hidden">
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-white/[0.04]">
            {filtered.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map(tx => (
              <div key={tx.id} className="px-2.5 sm:px-5 py-2.5 sm:py-4 hover:bg-white/[0.02] transition-colors group">
                <div className="flex items-start gap-2 sm:gap-3">
                  <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 mt-0.5 sm:mt-0 ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start sm:items-center justify-between gap-2">
                      ...
                      <p className={`font-semibold text-xs sm:text-sm shrink-0 ${tx.type === 'income' ? 'text-emerald-400' : 'text-red-400'}`}>
                        {tx.type === 'income' ? '+' : '-'}{formatUZS(tx.amount)}
                      </p>
                    </div>
                    <div className="flex items-center gap-1 sm:gap-2 text-[9px] sm:text-[11px] text-white/25 mt-0.5 flex-wrap">
                      ...
                      <span className="text-white/20">
                        <span className="hidden sm:inline">{formatDateTashkent(tx.date)} </span>{formatTimeTashkent(tx.date)}
                      </span>
                    </div>
                    {canModify(tx.admin, tx.date) && (
                      <div className="flex items-center gap-1 mt-1 sm:mt-1">
                        <button
                          ...
                          className="p-1 sm:p-1.5 rounded-lg hover:bg-amber-500/10 text-white/20 hover:text-amber-400 transition-all"
                          title="Tahrirlash"
                        >
                        ...
                        {deleteConfirm === tx.id ? (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-red-500/15 text-red-400 rounded-lg text-[9px] sm:text-[10px] font-medium border border-red-500/20"
                            >
                              Ha
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); setDeleteConfirm(null) }}
                              className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-white/[0.05] text-white/40 rounded-lg text-[9px] sm:text-[10px] font-medium border border-white/[0.06]"
                            >
                              Yo'q
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="p-1 sm:p-1.5 rounded-lg hover:bg-red-500/10 text-white/20 hover:text-red-400 transition-all"
                            title="O'chirish"
                          >
        ...
        {filtered.length > PAGE_SIZE && (
          <div className="flex items-center justify-between px-2.5 sm:px-5 py-2.5 sm:py-3 border-t border-white/[0.06]">
            <span className="text-[9px] sm:text-xs text-white/25">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <button
                ...
                disabled={page === 0}
                className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-medium bg-white/[0.03] text-white/40 border border-white/[0.06] disabled:opacity-30 hover:text-white/60 transition-all"
              >
                Oldingi
              </button>
              <button
